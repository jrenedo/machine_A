...
% r=R_m+2.95*10^-3; % [m] radius where we want to calculate the waveform (used in 2DFEA)
R_wave=r;
...
cp_func_adapted=(f_tempr+i*f_tempt);
...
Mat_ms_n=Mat_multiple_slots_n;


matrix=[Mat_ms_n(2:2:24,1), Mat_multiple_slots(2:2:24,1)]*1000;
figure
hArray = bar(matrix);
...
clear matrix
matrix=[Mat_ms_n(2:2:24,2), Mat_multiple_slots(2:2:24,2)]*1000;
figure
hArray = bar(matrix);
set(hArray(1),'FaceColor','w');
set(hArray(2),'FaceColor',[0.8 0.8 0.8]);
...
clear matrix
matrix=[Mat_ms_n(2:2:24,3), Mat_multiple_slots(2:2:24,3)]*1000;
figure
hArray = bar(matrix);
set(hArray(1),'FaceColor','w');
set(hArray(2),'FaceColor',[0.8 0.8 0.8]);
...
plot(time/180,x_ms(:,pos),'LineWidth',1.8,'Color',[0    0.4470    0.7410])
